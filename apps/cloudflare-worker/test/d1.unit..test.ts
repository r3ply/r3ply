...
describe('schema', () => {
  ...
  test('accept comment', async () => {
    const accept_result = (await state.viaEmail.accept('MESSAGE 123')).results[0]
    const select_result = (
      await env.TEST_DB.prepare('select created_utc from comments_via_email where message_id = ?')
        .bind('MESSAGE 123')
        .run<{ created_utc: string }>()
    ).results[0]
    expect(accept_result.ts_rcvd).toBe(String(Date.parse(select_result.created_utc + 'Z') / 1000))
  ...
  test('update deliverability', async () => {
    ...
    const select_deliverable = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_deliverable.results.length).toBe(1)
    expect(select_deliverable.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'deliverable' })
    await state.viaEmail.deliverable(accept_result.comment_id, 'undeliverable')
    const select_undeliverable = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_undeliverable.results.length).toBe(1)
    expect(select_undeliverable.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'undeliverable' })
  })
  test('update preparedness', async () => {
    const accept_result = (await state.viaEmail.accept('MESSAGE 123')).results[0]
    await state.viaEmail.prepared(accept_result.comment_id, 'prepared')
    const select_prepared = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_prepared.results.length).toBe(1)
    expect(select_prepared.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'prepared' })
    await state.viaEmail.prepared(accept_result.comment_id, 'unpreparable')
    const select_unpreparable = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_unpreparable.results.length).toBe(1)
    expect(select_unpreparable.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'unpreparable' })
  })
  test('update processability', async () => {
    ...
    const select_prepared = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_prepared.results.length).toBe(1)
    expect(select_prepared.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'processed' })
    await state.viaEmail.processed(accept_result.comment_id, 'unprocessable')
    const select_unpreparable = await env.TEST_DB.prepare('SELECT state, id FROM comments_via_email').run()
    expect(select_unpreparable.results.length).toBe(1)
    expect(select_unpreparable.results[0]).toStrictEqual({ id: accept_result.comment_id, state: 'unprocessable' })
  })
})
