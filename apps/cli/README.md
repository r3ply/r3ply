...
```
re comments generate --config static/.well-known/r3ply/config.toml --from bob@example.com --to integrate-w-next-version-of.spence.pages.dev@test.r3ply.com --subject https://integrate-w-next-version-of.spence.pages.dev/writing/email-as-a-commenting-system/ | \
curl --request POST 'http://localhost:8787/cdn-cgi/handler/email' \
  --url-query 'from=bob@example.com' \
