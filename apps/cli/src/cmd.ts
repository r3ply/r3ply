  ...
  export function generate_cmd(cwd: string) {
    ...
    const signet_cmd = generate_cmd
      ...
      .action(async (options: GenerateSignetCmdOpts) => {
        ...
        const signet = await lib_generate.signet(keys.signet_key, cli_system_config, {
          domain: options.site,
          r3ply: options.r3ply,
          issued: options.date,
          label: options.label,
    ...
    const email_cmd = generate_cmd
      ...
      .action(async (input: string | undefined, options: GenerateEmailCmdOpts) => {
        ...
        const email = Result.safe(
          ...
          lib_generate.email(site.domain, site.r3ply, { body: input, ...options }),
        )
...
export namespace cache {
  export type CacheStartOpts = {
    port: number
    interface: string
    reset: boolean
  }
  export function cache_cmd(cwd: string) {
    const cache_cmd = new Command('cache').description(
      'manage comment cache',
    )
    cache_cmd
      .command('serve')
      .description('serves cached comments')
      .option('--interface <INTERFACE>', 'Interface to bind on', "127.0.0.1")
      .option<number>('--port <PORT>', 'Which port to use', (str) => {
        const num = Number.parseInt(str)
        if (Number.isNaN(num)) throw new Error(`Port must be a number, received ${(str)}`)
        return num
      }, 2274)
