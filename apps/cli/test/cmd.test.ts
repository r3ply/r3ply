...
describe('config', () => {
  describe('validate', () => {
    ...
    test('valid r3ply config', async () => {
      mockfs({ '/.r3ply': {}, '/r3ply.config.toml': fs.readFileSync(path.join(__dirname, 'resources/minimum_r3ply_site_config.toml')).toString() })
    ...
    test('--config option', async () => {
      mockfs({ project: { src: {}, '.r3ply': {}, 'r3ply.config.toml': fs.readFileSync(path.join(__dirname, 'resources/minimum_r3ply_site_config.toml')).toString() }})
      const program = new Command()
