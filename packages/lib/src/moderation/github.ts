...
export function R3plyGithubBot<F extends typeof fetch>(github_pw: string, fetch: F): R3plyGithubBot {
  async function send(comment: string, context: CommentTemplateContext, siteConfig: R3plySiteConfig) {
    if (siteConfig.comments.email.moderation.type != 'github')
      throw new Error("Moderation type = 'github' is required to use GitHub Moderation")
    const gh_args = create_pr_args(comment, context, siteConfig.comments.email.moderation)
    ...
    return fetch(
      new Request('https://r3ply-github-app.spence.workers.dev/comments?strategy=GitHub:repo&open_pr=true', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${github_pw}` },
...
function parse_repo(repo_url: string) {
  let repo_as_url = Result.safe(() => new URL(repo_url)).expect(`Unable to parse GitHub repo as URL: ${repo_url}`)
  let [owner, repo] = Result.safe(() => repo_as_url.pathname.match(/^\/(.+?)\/(.+?)\/?$/)!.slice(1, 3)).expect(
    'Unable to parse GitHub owner/name of repo',
  )
  return { repo_owner: owner, repo_name: repo }
}

type R3plySiteConfigWithGithubModeration = R3plySiteConfig & { comments: { email: { moderation: { type: 'github' } } } }
type GithubModerationConfig = R3plySiteConfigWithGithubModeration['comments']['email']['moderation']

// separate function so it can be tested
function create_pr_args(comment: string, context: CommentTemplateContext, github_config: GithubModerationConfig) {
  ...
  let new_comment_filepath = tera(github_config['file_path_{}'], sanitized_context)
...
if (import.meta.vitest) {
  ...
  test('parse_repo', () => {
    expect(parse_repo('https://github.com/asimpletune/spenc.es')).toStrictEqual({ repo_owner: 'asimpletune', repo_name: 'spenc.es' })
    expect(() => parse_repo('github.com/asimpletune/spenc.es')).toThrowError(/Unable to parse GitHub repo as URL/)
    expect(() => parse_repo('https://github.com/')).toThrowError(/Unable to parse GitHub owner\/name of repo/)
    expect(parse_repo('https://github.com/a/b/c/d/e/f')).toStrictEqual({ repo_owner: 'a', repo_name: 'b/c/d/e/f' })
  })
  test('create_pr_args', () => {
    ...
    const github_moderation: GithubModerationConfig = {
      ...
      'pr_body_{}': 'this is a PR to merge comment from user {{ comment.author_7 }}, with content: \n> {{ comment.txt }}',
    }
    ...
    expect(result).toStrictEqual({
      ...
      pr: {
        ...
        msg_body: 'this is a PR to merge comment from user 7654321, with content: \n' + '> this is a comment',
      },
    })
  })
}
