...
export const signet = {
  ...
  description:
    'Requires a separate entry for each `site` x `r3ply` (and `issued`) combination.',
  type: 'object',
  ...
  properties: {
    domain: {
      title: 'Site domain',
      description:
        'The domain that this configuration applies to. Hostname only.',
      type: 'string',
      format: 'hostname',
    },
    r3ply: {
    ...
    issued: {
      ...
      description:
        'Used as a key identifier, e.g. for signet rotations and versioning of signet key.',
      type: 'string',
      format: 'date',
      examples: ['2025-08-22'],
    },
    label: {
      title: 'Site label',
      description:
        'A human readable label of a site. Useful for filtering further downstream.',
